...
|---|---|---|---|---|
| B1 | **Modifiche avviso non salvate** | Modificando un avviso di pagamento (periodo, importo, scadenza) le modifiche non persistevano su Supabase dopo il salvataggio. Fix applicato: `EditCedolinoDialog` ora include il campo importo + `onSave` passa `total` aggiornato. | `RentNoticesScreen.kt` → `EditCedolinoDialog` | ✅ Risolto e verificato |
| B2 | **Avviso eliminato ancora visibile nel Report** | Dopo aver eliminato un cedolino, il Report mostrava ancora i dati. Fix `refresh()` + cascade delete applicati (vedi B7). Verificato su device: eliminazione cedolino pagato → sparisce correttamente da Report e totali. | `SupabaseRentRepository.kt` → `deleteCedolino()` | ✅ Risolto e verificato |
...
| B7 | **Payment orfano dopo eliminazione cedolino** | Fix applicato in `deleteCedolino()`: (1) DELETE per `cedolino_id=eq.$id`, (2) DELETE time-scoped ±45gg per payment manuali senza `cedolino_id`. Payment legacy già esistente (`c5e80172...` — Cedolino Luglio 2026 €1000) eliminato manualmente da Supabase SQL Editor il 15/05/2026. | `SupabaseRentRepository.kt` riga 304-348 | ✅ Risolto e verificato |
